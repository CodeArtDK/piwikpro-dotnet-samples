...
public partial class Trends
{
    ...
    private List<TrendData>? _data;
    private List<ChartSeries> _chartSeries = new();
    private List<ChartSeries> _sessionsSeries = new();
    private List<ChartSeries> _pageViewsSeries = new();
    ...
    private async Task LoadData()
    {
        ...
        try
        {
            _data = await DataService.GetTrendsDataAsync(_startDate, _endDate, _filters);
            ...
            if (_data != null && _data.Any())
            {
                ...
                _chartSeries = new List<ChartSeries>
                {
                    ...
                    {
                        Name = "Page Views",
                        Data = _data.Select(x => (double)x.PageViews).ToArray()
                    }
                };

                _sessionsSeries = new List<ChartSeries>
                {
                    new ChartSeries
                    {
                        Name = "Sessions",
                ...
                _pageViewsSeries = new List<ChartSeries>
                {
                    new ChartSeries
                    {
                        Name = "Page Views",
            ...
            {
                Snackbar.Add("No trend data found for the selected date range.", Severity.Info);
            }
        }
        catch (Exception ex)
        {
            Snackbar.Add($"Error loading trend data: {ex.Message}", Severity.Error);
